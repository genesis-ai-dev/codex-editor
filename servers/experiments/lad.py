...
def character_scoring(codex_results, bible_results):
    ...
    codex_char_diffs = [similarity_ratio(codex_chars[0], result) for result in codex_results]
    bible_char_diffs = [similarity_ratio(bible_chars[0], result) for result in bible_results]
...
def test_search(query: str, n_samples, codex, bible):
    english_sentences = [
        "The quick brown fox jumped over the lazy dog", # Truth, the quick brown fox jumps over the lazy dog.
