 ...
 * Moved from SQL custom function to JS since the native sqlite3 binding doesn't support create_function.
 */
...
export default async function registerClientOnRequests(client: LanguageClient, db: AsyncDatabase) {
    try {
        ...
        client.onRequest(
            ...
            async ({ word, caseSensitive = false }: { word: string; caseSensitive: boolean }) => {
                try {
                    ...
                    const entry = await getEntry(db, word, caseSensitive);
                    return { exists: entry };
                } catch (error) {
        ...
        client.onRequest(CustomRequests.GetSuggestions, async (word: string) => {
            try {
                ...
                // Fetch candidate words from DB using LIKE, then sort by Levenshtein in JS.
                // Native sqlite3 doesn't support create_function(), so Levenshtein runs in JS.
                ...
                // Sort by Levenshtein distance in JavaScript
                const lowerWord = word.toLowerCase();
                const sorted = rows
                    .map((r) => ({
                        word: r.head_word,
                        distance: levenshtein(r.head_word.toLowerCase(), lowerWord),
                    }))
                    .sort((a, b) => a.distance - b.distance)
                    .slice(0, 100);

                return sorted.map((s) => s.word);
            } catch (error) {
