            ...
            for (const alignedCell of message.alignedContent) {
                if (alignedCell.isParatext) {
                    ...
                    if (existingCell && existingCell.value && existingCell.value.trim() !== "") {
                        ...
                        // Update empty cell with new content (even if new content is also empty)
                    ...
                    console.warn(`[Translation Import] Aligned cell with id ${alignedCell.importedContent.id} has no notebookCell property - will be skipped`);
                }
            }
