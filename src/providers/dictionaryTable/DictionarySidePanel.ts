...
export class DictionarySidePanel implements vscode.WebviewViewProvider {
    ...
    private async updateWebviewData() {
      ...
      this._view?.webview.postMessage({ command: "sendData", data: dictionary });
  }
    ...
    private getWebviewContent(webview: vscode.Webview): string {
        const stylesUri = getUri(webview, this.extensionUri, ['webviews', 'dictionary-side-panel', 'dist', 'assets', 'index.css']);
        const scriptUri = getUri(webview, this.extensionUri, ['webviews', 'dictionary-side-panel', 'dist', 'assets', 'index.js']);
              ...
              const data = message.data;
                switch (message.command) {
                  ...
                  case "showDictionaryTable": {
                    vscode.commands.executeCommand('dictionaryTable.showDictionaryTable').then(() => {
                        console.log('Dictionary Table webview displayed');
                    }, (err) => {
                        console.error(err);
                    });
                    return;
