...
export const downloadSBTranslationResources = async ({
  ...
  try {
    ...
    const downloadResourceFolder = resourcesFolder.with({
      path: path.join(resourcesFolder.path, downloadProjectName),
    });
    ...
    const zipUri = Uri.from({
      path: path.join(resourcesFolder.path, `${projectResource?.name}.zip`),
      scheme: "file",
    });
    ...
    for (const key of keys) {
      ...
      if (item.dir) {
        await workspace.fs.createDirectory(
          Uri.file(path.join(downloadResourceFolder.path, item.name))
        );
      } else {
