...
export async function importLocalUsfmSourceBible() {
    ...
    bibleData.metadata.data.corpusMarker = testament === "OT" ? "Old Testament" :
        testament === "NT" ? "New Testament" :
            undefined;

    try {
        vscode.workspace.fs.writeFile(bibleFilePath, Buffer.from(JSON.stringify(bibleData, null, 2), 'utf-8'));
...
export async function createProjectNotebooks({
    ...
    for (const book of allBooks) {
        try {
            ...
            vscode.window.showErrorMessage(`Error encountered while creating notebook for book ${book}. Error: ${e}`);
        }
    }
    await Promise.all(notebookCreationPromises);
}