...
suite("Provider + Merge Integration - multi-user multi-field edits", () => {
    ...
    test("last edit to isMissing wins during merge for audio attachments", async () => {
        ...
        const localTheirsUri = await createTempCodexFile(
            `merge-audio-missing-theirs-${Date.now()}-${Math.random().toString(36).slice(2)}.codex`,
            JSON.parse(JSON.stringify(codexSubtitleContent))
        );
        ...
        // Reload both documents to reflect updatedAt timestamps
