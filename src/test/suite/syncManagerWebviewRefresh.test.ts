...
suite('SyncManager Webview Refresh Tests', () => {
    ...
    test('webview refresh skipped when no codex files changed', async function () {
        this.timeout(15000);

        // Mock stageAndCommitAllAndSync to return sync result with only non-codex files
        const mergeModule = await import('../../projectManager/utils/merge');
        const syncStub = sinon.stub(mergeModule, 'stageAndCommitAllAndSync').resolves({
            success: true,
            changedFiles: ['some-file.txt', 'another-file.json'],
            conflictFiles: [],
            newFiles: [],
            deletedFiles: [],
            totalChanges: 2,
        ...
        // Verify refreshWebviewsForFiles was NOT called (since no .codex files changed)
        // Note: Due to async execution, we check that if it was called, it was with empty array
        // or verify it wasn't called at all
        if (refreshSpy.called) {
            const calls = refreshSpy.getCalls();
            const codexFilesInCalls = calls.some(call => {
                const files = call.args[0] as string[];
                return files.some(f => f.endsWith('.codex'));
            });
            assert.ok(!codexFilesInCalls, 'refreshWebviewsForFiles should not be called with codex files when none changed');
        }
