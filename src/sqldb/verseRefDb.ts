...
export function populateVerseRefsFTS5FromMainTable(db: Database): void {
    ...
    try {
        ...
        const tableExistsStmt = db.prepare(`
            ...
            WHERE type='table' AND name='verse_refs'
        `);
        ...
        if (!tableExists || !tableExists.name) {
            console.log("Verse refs table does not exist, skipping FTS5 population");
            return;
        }
        
        // Check if the main table has data
        const checkStmt = db.prepare("SELECT COUNT(*) as count FROM verse_refs");
        checkStmt.step();
        const mainTableCount = checkStmt.getAsObject().count as number;
        checkStmt.free();
        
        if (mainTableCount === 0) {
            console.log("Verse refs table is empty, skipping FTS5 population");
            return;
        }
        ...
        console.log("Continuing with empty verse refs FTS5 table");
    }
}
