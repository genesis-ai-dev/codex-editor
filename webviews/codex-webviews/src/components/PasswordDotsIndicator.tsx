...
export const PasswordDotsIndicator: React.FC<PasswordDotsIndicatorProps> = ({
        ...
        sourcesToCheck.forEach((source) => {
            for (let sourceStart = 0; sourceStart <= source.length - 3; sourceStart++) {
                for (let sourceEnd = sourceStart + 3; sourceEnd <= source.length; sourceEnd++) {
                    ...
                    for (
                        let passStart = 0;
                        passStart <= lowerPassword.length - sourceSubstring.length;
                        passStart++
                    ) {
                        if (
                            lowerPassword.substring(
                                passStart,
                                passStart + sourceSubstring.length
                            ) === sourceSubstring
                        ) {
                            matches.push({
                                start: passStart,
                                end: passStart + sourceSubstring.length,
                            });
                        }
                    }
                }
    ...
    const renderDots = () => {
        ...
        for (let i = 0; i < currentLength; i++) {
            const isInProblematicMatch = problematicMatches.some(
...
export const validateVisualPassword = (
    ...
    if (password.length >= 3) {
        ...
        sourcesToCheck.forEach((source) => {
            ...
            for (
                let sourceStart = 0;
                sourceStart <= source.length - 3 && !hasMatch;
                sourceStart++
            ) {
                for (let sourceEnd = sourceStart + 3; sourceEnd <= source.length; sourceEnd++) {
                    const sourceSubstring = source.substring(sourceStart, sourceEnd);
