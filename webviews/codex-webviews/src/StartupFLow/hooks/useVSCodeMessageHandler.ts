...
export const useVSCodeMessageHandler = (
    ...
    const messageHandler = useCallback((event: MessageEvent<SourceUploadResponseMessages>) => {
        const message = event.data;
        console.log("Received message:", message);

        switch (message.command) {
            case "checkWorkspaceState":
                setIsWorkspaceOpen(message.isWorkspaceOpen);
                break;
            ...
            case "updateAuthState": {
                if (isWorkspaceOpen) {
                    setWorkflowState((prev) => ({
                        ...prev,
                        step: message.authState.isAuthExtensionInstalled
                            ? "auth"
                            : "project-select",
                    ...
                    setWorkflowState((prev) => ({
                        ...prev,
                        step: message.authState.isAuthExtensionInstalled ? "auth" : "type-select",
                        authState: message.authState,
                    }));
                }
                break;
