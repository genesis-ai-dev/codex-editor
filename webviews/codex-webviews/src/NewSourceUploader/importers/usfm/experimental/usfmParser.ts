...
export async function parseUsfmFile(
    ...
    // Track cells created before first chapter (for milestone 0)
    let cellsBeforeFirstChapter = 0;
    ...
    function finishCurrentVerse() {
        ...
        const { cellId, metadata: cellMetadata } = createVerseCellMetadata({
            bookCode,
            bookName,
            fileName: file.name,
            chapter: cellChapter,
        ...
        // Update lineMappings for this verse to use the UUID cellId
        // Find all lineMappings entries that reference this verse and update them
        const verseRef = `${bookCode} ${cellChapter}:${verseNumber}`;
        for (const mapping of lineMappings) {
            if (mapping.cellId === verseRef) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (trimmedLine.startsWith('\\')) {
            ...
            if (markerMatch) {
                ...
                // Headers before first chapter marker are assigned to chapter 0 (for milestone detection)
                const cellChapter = seenFirstChapter ? currentChapter : 0;
                ...
                // Create cell metadata (generates UUID internally)
                const { cellId, metadata: cellMetadata } = createParatextCellMetadata({
                    bookCode,
                    bookName,
                    fileName: file.name,
                    chapter: cellChapter,
