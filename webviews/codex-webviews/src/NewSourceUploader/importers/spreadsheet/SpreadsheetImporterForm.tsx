    ...
    const handleImport = async () => {
        ...
        try {
            if (isTranslationImport) {
                ...
                // DON'T filter out empty rows - preserve them to maintain row order
                const importedContent: ImportedContent[] = parsedData.rows.map((row, index) => {
                    ...
                    return {
                        id,
                        content: row[parseInt(targetColumnIndex!)] || "", // Use empty string for blank cells
                        rowIndex: index,
                    };
                });
                ...
                const sourceCells = parsedData.rows
                    ...
                    .map((row, index) => {
                        ...
                        return {
                            id,
                            content: row[parseInt(sourceColumnIndex!)] || "", // Use empty string for blank cells
                            images: [],
                            metadata: {
                                id,
                ...
                const notebookPair: NotebookPair = {
                    ...
                    codex: {
                        ...
                        cells: sourceCells.map((cell) => ({
                            ...cell,
                            // Always create empty target cells for Source imports
                            // This gives users control to use AI auto-completion first,
                            // or import target data later via Target importer
                            content: "",
                            // Preserve metadata including cellLabel
                        })),
                        metadata: {
                            id: parsedData.filename,
                            originalFileName: selectedFile!.name,
